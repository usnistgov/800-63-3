---
layout: page
title: "Section 5. Authenticator and Verifier Requirements"
description: "Section 5. Authenticator and Verifier Requirements"
---
...
Verifiers SHALL store memorized secrets in a form that is resistant to offline attacks. Secrets SHALL be hashed with a *salt* value using an approved hash function such as PBKDF2 as described in [[SP800-132]](references.html#SP800-132). The salt value SHALL be a 32 bit (or longer) random value generated by an approved random number generator and is stored along with the hash result. At least 10,000 iterations of the hash function SHOULD be performed; this iteration count SHOULD be doubled every year after 2016. A keyed hash function (e.g., HMAC), with the key stored separately from the hashed authenticators (e.g., in a hardware security module) SHOULD be used to further resist dictionary attacks against the stored hashed authenticators.
...
Verifiers SHALL store look-up secrets in a form that is resistant to offline attacks. Secrets having less than 64 bits of entropy SHALL be hashed with a "salt" value using an approved hash function as described in [[SP 800-132]](references.html#sp800-132). The "salt" value SHALL be a 32 bit (or longer) random value generated by an approved random number generator and is stored along with the hash result. A keyed hash function (e.g., HMAC [[FIPS198-1]](references.html#FIPS198-1)), with the key stored separately from the hashed authenticators (e.g., in a hardware security module) SHOULD be used to further resist dictionary attacks against the stored hashed authenticators. Secrets having at least 64 bits of entropy MAY simply be hashed using an approved hash function.
